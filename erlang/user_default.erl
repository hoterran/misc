...
p() ->
	lists:foreach(fun(S) -> 
		io:format("| ~p    |     ~p     |    ~p     | ~n", 
			[S, 
			proplists:get_value(registered_name, erlang:process_info(S)),
			proplists:get_value('$ancestors', proplists:get_value(dictionary, erlang:process_info(S)))
			]) 
		end, erlang:processes()),
		io:format("~p~n", [length(erlang:processes())]).
