...
start() ->
	...
	State = #state{tid=1, packetnumber=0, listen=ListenSocket, qps=0, max_qps=2, node_qps=0},
	start_child(State).

start_child(State) ->
	io:format("haha ~p", [self()]),
...
handle_client_connect(accept, State=#state{listen=ListenSocket, tid=Tid, packetnumber=PacketNumber}) ->
	...
	{ok, Client} ->
		%%?
		spawn(?MODULE, start_child, [State#state{tid=Tid+1, packetnumber=0, qps=0, node_qps=0}]),
...
handle_backend_authresult(read_authresult, State=#state{username=User, client=Client, server=Server}) ->
	...
	case gen_tcp:recv(Server, 0) of
		{ok, RecvBinary} ->
			...
			mysql_proxy_qps:start_link(self(), binary_to_list(User)),
			mysql_proxy_qps:add_conn(binary_to_list(User)),
...
handle_client_command(read_cmd, State = #state{client=Client, server=Server, username=User, qps=Qps, max_qps=MaxQps, cbuffer=Buf, node_qps=NodeQps, conns=Conns}) ->
	%io:format("handle_client_command----------------------------------------------~n~n"),
	case decode_packet(Buf) of 
		{PacketLength, PacketNumber, Binary} ->
			%io:format("recv ~p~n", [RecvBinary]),
			%%TODO just check data 
			%io:format("~p~p", [Qps, MaxQps]),
			if Qps >= MaxQps  ->
				%% 0.01 TODO
				%% 100 = 1000ms / 10 stepseconds
				SleepTime = round((1000 / MaxQps - 1000 / Qps) * 0.9 *  Conns) ,
				timer:sleep(SleepTime),
				io:format("~p qps limits ~p ~p ~p s[~p] ~n", [self(), Qps, MaxQps,Conns, SleepTime]);
			true ->
				io:format("[~p] qps is ok ~p ~p ~p ~n", [self(), Qps, MaxQps, NodeQps])
			end,
			
			{CmdType, Sql} = decode_packet(cmd, Binary),
			%io:format("client send proxy: [~p][~p]~n", [CmdType, Sql]),
			gen_tcp:send(Server, Buf),
			%io:format("proxy send backend: [~p][~p]~n", [CmdType, Sql]),
			inet:setopts(Server, [{active, once}]),
			{next_state, handle_backend_result, State#state{cbuffer = <<>>, node_qps=NodeQps+1}};
		_ ->
			io:format("half data "),
			{next_state, handle_client_command, State}
	end.

handle_backend_result(read_result, State = #state{client=Client, server=Server, sbuffer=Buf}) ->
	%io:format("handle_backend_result---------------------------------------------~n~n"),
	%%TODO possible half data
	case decode_packet(multi, Buf) of 
		{_, _, Binary}  ->
			...
			gen_tcp:send(Client, Buf),
			%io:format("proxy send client: [~p]~n", [RecvBinary]),
			inet:setopts(Client, [{active, once}]),
			{next_state, handle_client_command, State#state{sbuffer= <<>>}};
		_ ->
			io:format("half data "),
...
%%
handle_info({putqps, Qps, Conns}, StateName, State) ->
	%%io:format("put Qps [~p]~n", [Qps]),
	%%nodes qps
	{next_state, StateName, State#state{qps=Qps, conns=Conns}};

handle_info({getqps, Pid}, StateName, State=#state{node_qps=NodeQps}) ->
	io:format("get Qps ~p ~n", [NodeQps]),
	Pid ! {qps, NodeQps},
	%%rest node qps
	{next_state, StateName, State#state{node_qps=0}};

handle_info({tcp, Server, Data}, StateName, 
	State=#state{server=Server, sbuffer=Sbuffer}) ->
	%io:format("msg from backend~n"),
	gen_fsm:send_event(self(), read_result),
	{next_state, StateName, State#state{sbuffer= <<Sbuffer/binary, Data/binary>>}};

handle_info({tcp, Client, Data}, StateName,
	State=#state{client=Client, cbuffer=Cbuffer}) ->
	%io:format("msg from client ~n"),
	gen_fsm:send_event(self(), read_cmd),
	{next_state, StateName, State#state{cbuffer= <<Cbuffer/binary, Data/binary>>}};

handle_info({tcp_closed, Server}, StateName,
	State=#state{server=Server, username=User}) ->
	io:format("close from backend~n"),
	mysql_proxy_qps:remove_conn(binary_to_list(User)),
	{stop, normal, State};

handle_info({tcp_closed, Client}, StateName,
	State=#state{client=Client}) ->
	io:format("close from client ~n"),
	{stop, normal, State};
